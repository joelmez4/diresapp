...
<div class="column is-9" id="appDesnutricionCronicaNinos">
        ...
        <table id="iseqchart" class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
          <thead>
            <tr>
              ...
              <th><abbr title="D. Cronico">D. Cronico</abbr></th>
              <th><abbr title="Excluido">Excluido</abbr></th>
              <th><abbr title="Normal">Normal</abbr></th>
              <th><abbr title="Total General">Total General</abbr></th>
              <th><abbr title="% DC">% DC</abbr></th>
          ...
          <tbody>
            <tr v-for="(da,index) in dataAN">
              ...
              <td>
                <a class="button is-outlined is-small" @click="launch('cronico', index)">@{{da.sum_dcronico}}</a>
              </td>
              <td>
                <a class="button is-outlined is-small" @click="launch('excluido', index)">@{{da.sum_excluido}}</a>
              </td>
              <td>
                <a class="button is-outlined is-small" @click="launch('normal', index)">@{{da.sum_normal}}</a>
              </td>

              <td>@{{da.total_general}}</td>
              <td v-bind:style="{background: da.dc_color}">@{{da.dc}} %</td>
            ...
            <tr style="background: #50A87D">
              <th>#</th>
              <td>
                <strong><a href="#" title="Selección"></a></strong>
              </td>
              <td>
                @{{totalDCronico}}
              </td>
              ...
              <td>@{{totalGeneral}}</td>
              <td v-bind:style="{background: totalDCColor}">@{{totalDC}} % </td>
              <td></td>
              <td></td>
            </tr>
          </tbody>
...
<section class="info-tiles">
    <div class="tile is-ancestor has-text-centered">
        ...
        <div class="tile is-parent">
            <article class="tile is-child box">
                <p class="title">@{{totalGeneral}}</p>
                <p class="subtitle">Total Tamizados</p>
            </article>
        </div>
        <div class="tile is-parent">
            <article class="tile is-child box">
                <p class="title">@{{totalDCronico}}</p>
                <p class="subtitle">Total D.Cronico</p>
            </article>
        </div>
        <div class="tile is-parent">
            <article class="tile is-child box" v-bind:style="{background: totalDCColor}">
                <p class="title">@{{totalDC}} %</p>
      ...
      <div class="modal-background"></div>
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Detalles Anemia: @{{idDCronica}}</p>
            <button @click="close" class="delete" aria-label="close"></button>
          </header>
          <section class="modal-card-body">
            <!-- Content ... -->
            <div v-if="idDCronica === 'cronico'">
              <table id="iseqchart" class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                <thead>
                  <tr>
                    <th><abbr title="Número">N°</abbr></th>
                ...
                <tbody>
                  <tr v-for="(da,index) in detalleDCronica.data_dcronico">
                    <td>@{{index + 1}}</td>
            ...
            <div v-if="idDCronica === 'excluido'">
              <table id="iseqchart" class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                ...
                <tbody>
                  <tr v-for="(da,index) in detalleDCronica.data_excluido">
                    <td>@{{index + 1}}</td>
                    <td><a class="button is-outlined is-small" @click="verDetalleDNI(da)">@{{da}}</a></td>
                  </tr>
                </tbody>
            ...
            <div v-if="idDCronica === 'normal'">
              <table id="iseqchart" class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                ...
                <tbody>
                  <tr v-for="(da,index) in detalleDCronica.data_normal">
                    <td>@{{index + 1}}</td>
                    <td><a class="button is-outlined is-small" @click="verDetalleDNI(da)">@{{da}}</a></td>
                  </tr>
                </tbody>
    ...
    <div class="modal" v-bind:class="{'is-active':isActive2}">
      <div class="modal-background"></div>
        <div class="modal-card">
          <header class="modal-card-head">
            <p class="modal-card-title">Detalles DNI | @{{idDCronica}}</p>
...
<script src="{{ asset('/public/js/dc/appDesnutricionCronicaNinos.js') }}"></script>
